...
foreach ($person_ids as $id) {
    ...
    if(count($person->children['person_instance']) > 0){
        ...
        foreach( $person->children['person_instance'] as $person_instance ) {
            ...
            if ($provider_instance->getField('academic_year')->getDBValue() == 'academic_year|year_1' || $provider_instance->getField('academic_year')->getDBValue() == 'academic_year|year_2' || $provider_instance->getField('academic_year')->getDBValue() == 'academic_year|1') {
            ...
            }elseif ($provider_instance->getField('academic_year')->getDBValue() == 'academic_year|2' || $provider_instance->getField('academic_year')->getDBValue() == 'academic_year|3' || $provider_instance->getField('academic_year')->getDBValue() == 'academic_year|4') {
            	$index = explode('|', $provider_instance->getField('academic_year')->getDBValue());

            	if ($index[1] == '2') {
            		$semester_index = explode('|', $provider_instance->getField('semester')->getDBValue());
            		$semester_index_no = explode('_', $semester_index[1]);
            		if ($semester_index_no[1] == '1') {
            			if (!array_key_exists(10, $sort_ids)){
			                $sort_ids[10] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|first';
			            }
			            else{
			                if (!array_key_exists(11, $sort_ids)) {
			                    $sort_ids[11] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|retry';
			                }else{
			                    $sort_ids[12] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|final';
			                }
			            }
            		}
            		if ($semester_index_no[1] == '2') {
            			if (!array_key_exists(13, $sort_ids)){
			                $sort_ids[13] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|first';
			            }
			            else{
			                if (!array_key_exists(14, $sort_ids)) {
			                    $sort_ids[14] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|retry';
			                }else{
			                    $sort_ids[15] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|final';
			                }
			            }
            		}  
		        }
		        ...
		        if ($index[1] == '3') {
            		...
            		$semester_index_no = explode('_', $semester_index[1]);
            		if ($semester_index_no[1] == '1') {
            			if (!array_key_exists(16, $sort_ids)){
			                $sort_ids[16] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|first';
			            }
			            else{
			                if (!array_key_exists(17, $sort_ids)) {
			                    $sort_ids[17] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|retry';
			                }else{
			                    $sort_ids[18] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|final';
			                }
			            }
            		}
            		if ($semester_index_no[1] == '2') {
            			if (!array_key_exists(19, $sort_ids)){
			                $sort_ids[19] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|first';
			            }
			            else{
			                if (!array_key_exists(20, $sort_ids)) {
			                    $sort_ids[20] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|retry';
			                }else{
			                    $sort_ids[21] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|final';
			                }
			            }
            		}
		        ...
		        if ($index[1] == '4') {
            		...
            		$semester_index_no = explode('_', $semester_index[1]);
            		if ($semester_index_no[1] == '1') {
            			if (!array_key_exists(22, $sort_ids)){
			                $sort_ids[22] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|first';
			            }
			            else{
			                if (!array_key_exists(23, $sort_ids)) {
			                    $sort_ids[23] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|retry';
			                }else{
			                    $sort_ids[24] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|final';
			                }
			            }
            		}
            		if ($semester_index_no[1] == '2') {
            			if (!array_key_exists(25, $sort_ids)){
			                $sort_ids[25] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|first';
			            }
			            else{
			                if (!array_key_exists(26, $sort_ids)) {
			                    $sort_ids[26] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|retry';
			                }else{
			                    $sort_ids[27] = $person_instance->getField('id')->getDBValue() . ',' . 'exam_try|final';
			                }
			            }
            		}
    		...
    		if(count($person_instance->children['person_exam']) > 0){
                foreach( $person_instance->children['person_exam'] as $person_exam ) {
	                	...
	                	$where_data = array(
	                                    ...
	                                    'operand' => array(
	                                                ...
	                                                3 => array(
	                                                    ...
	                                                    'style' => 'equals',
	                                                    'data' => array(
	                                                            'value' => $provider_instance->getField('semester')->getDBValue(),
	                                                            )
	                                                    ),
	                                                ),
	                                    );
			        ...
			        if ($person_exam->results != '') {
			        	if (strcasecmp($course_name, 'Practical I') == 0 || strcasecmp($course_name, 'Practical II') == 0 || strcasecmp($course_name, 'Practical III') == 0 || strcasecmp($course_name, 'Practical IV') == 0 || strcasecmp($course_name, 'Practical V') == 0){
			        		$person_exam->results = $person_exam->results * 2;
			        	}
			            ...
			            if ($person_exam->results <= 100 ){
			                if($person_exam->results >= 80){
			                    ...
			                    if ($where_data){
			                        ...
			                        if (($person_exam->results == 48 ) || ($person_exam->results == 49)) {
			                            if (strcasecmp($course_name, 'Oral and Practical') == 0 || strcasecmp($course_name, 'Practicals') == 0 || strcasecmp($course_name, 'Practical I') == 0 || strcasecmp($course_name, 'Practical II') == 0 || strcasecmp($course_name, 'Practical III') == 0 || strcasecmp($course_name, 'Practical IV') == 0 || strcasecmp($course_name, 'Practical V') == 0) {
			                ...
			                $person_exam->results->setInvalid( "The total value from the sections exceeds 100. Please correct the values form the section results");
			            }
			        }
