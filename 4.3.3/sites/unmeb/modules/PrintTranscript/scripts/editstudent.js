...
function changeFinalExamGrade(node,exam_type,person_scheduled_training_course,exam,passing_score,assessment) {
    node = $(node);
    ...
    if(assessment=='assessment|exam' || assessment=='')
    {
    ...
    else if(assessment=='assessment|evaluation')
    {
    if (score == 'evaluation|pass') {
	color = 'green';
    } else {
	color = 'red';
    }
	...
	var url = 'index.php/training-course/exam';
        var req = new Request.HTML({
            method: 'post',
            url: url,
            data: { 'id':exam, 'parent' : person_scheduled_training_course, 'exam_type':exam_type,'score': score, 'action':'updatescore','submit_type':'save','assessment':assessment},
