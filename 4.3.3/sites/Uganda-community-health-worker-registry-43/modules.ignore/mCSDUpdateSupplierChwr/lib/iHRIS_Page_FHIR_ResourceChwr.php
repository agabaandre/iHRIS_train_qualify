...
class iHRIS_Page_FHIR_ResourceChwr extends I2CE_Page {
	...
	protected function loadData_Practitioner($uuid, &$top) {
		...
		if (!$person_id) {
			...
			$data['id_system_national'] = 'https://www.immigration.go.ug/';
			$data['id_code_national'] = $person->national_id;
			$data['active'] = 'true';
			$data['family'] = $person->surname;
			$data['given'] = array($person->firstname, $person->othername);
			//contacts comming from person page
			$data['telecom'] = array(
			...
			$data['telecom']['mobile'] = array();
			$data['telecom']['work']['phone'][] = $person->mobile_phone;
			$data['telecom']['mobile']['phone'][] = $person->alt_telephone;
	...
	protected function getUpdates_Practitioner(&$top) {
		...
		$required_forms = array("person", "demographic");
		...
		$params = array();
		if ($this->since) {
			$where = "WHERE GREATEST(person.last_modified, IFNULL(demographic.last_modified,'0000-00-00 00:00:00')) >= ?";
			$params = array($this->since);
		}

		$qry = "SELECT person.csd_uuid AS uuid, DATE_FORMAT(person.last_modified, '%Y-%m-%d %T') AS lastupdated, person.id, person.national_id AS national_id, person.surname AS family, person.firstname AS given, person.othername AS given2, person.gender, DATE_FORMAT(person.birth_date,'%Y-%m-%d') AS birthdate, person.alt_telephone AS home_phone, person.mobile_phone AS home_mobile FROM hippo_person AS person $where ORDER BY lastupdated ASC";
		...
		try {
			...
			while ($row = $stmt->fetch()) {
				...
				$data['id_code'] = $row->id;
				$data['id_system_national'] = 'https://www.immigration.go.ug/';
				$data['id_code_national'] = $row->national_id;
				$data['active'] = 'true';
				...
				if ($row->home_mobile) {
					$data['telecom']['work']['phone'][] = $row->home_mobile;
				}
				if ($row->home_phone) {
					$data['telecom']['mobile']['phone'][] = $row->home_phone;
				}
	...
	protected function create_Practitioner($data, &$top) {
		...
		if ($this->useJSON) {
			...
			if ((array_key_exists('id_system', $data) && array_key_exists('id_code', $data)) || (array_key_exists('id_system_national', $data) && array_key_exists('id_code_national', $data))) {
				$top['identifier'] = array();
				$top['identifier'][] = array('system' => $data['id_system'], 'value' => $data['id_code']);
				$top['identifier'][] = array('system' => $data['id_system_national'], 'value' => $data['id_code_national']);
			}
			...
			if (array_key_exists('telecom', $data)) {
				$top['telecom'] = array();
				$top['telecom'][] = array('resourceType' => 'ContactPoint', 'system' => 'work', 'use' => 'phone', 'value' => $data['telecom']['work']['phone'][0]);
				$top['telecom'][] = array('resourceType' => 'ContactPoint', 'system' => 'mobile', 'use' => 'phone', 'value' => $data['telecom']['mobile']['phone'][0]);
			}
			...
			if ((array_key_exists('id_system', $data) && array_key_exists('id_code', $data)) || (array_key_exists('id_system_national', $data) && array_key_exists('id_code_national', $data))) {
				...
				if ((array_key_exists('id_system_national', $data) && array_key_exists('id_code_national', $data))) {
					$sys = $this->doc->createElement("system");
					$sys->setAttribute('value', $data['id_system_national']);
					$ident->appendChild($sys);
					$code = $this->doc->createElement("code");
					$code->setAttribute('value', $data['id_code_national']);
					$ident->appendChild($code);
				}
			...
			if (array_key_exists('telecom', $data)) {
				...
				$sys = $this->doc->createElement("system");
				$sys->setAttribute('value', "work");
				$tele->appendChild($sys);
				$u = $this->doc->createElement("use");
				$u->setAttribute('value', "phone");
				$tele->appendChild($u);
				$value = $this->doc->createElement('value');
				$value->setAttribute('value', $data['telecom']['work']['phone'][]);
				$tele->appendChild($value);

				$sys2 = $this->doc->createElement("system");
				$sys2->setAttribute('value', "mobile");
				$tele->appendChild($sys2);
				$u2 = $this->doc->createElement("use");
				$u2->setAttribute('value', "phone");
				$tele->appendChild($u2);
				$value2 = $this->doc->createElement('value');
				$value2->setAttribute('value', $data['telecom']['mobile']['phone'][]);
				$tele->appendChild($value2);
	...
	protected function loadData_Location($uuid, &$top) {
		...
		if (!$fac_id) {
			...
			$data['id_system_dhis2'] = 'https://hmis2.health.go.ug/';
			$data['id_code_dhis2'] = $facility->getField('dhis_orgunit')->getDBValue();
	...
	protected function getUpdates_Location(&$top) {
		...
		$queries = array(
			'facility' => "SELECT facility.csd_uuid as uuid, DATE_FORMAT(facility.last_modified, '%Y-%m-%d %T') AS lastupdated, facility.id, facility.name, facility.dhis_orgunit, facility.facility_type, ft.name AS facility_type_name, IFNULL(county.csd_uuid, IFNULL(district.csd_uuid, IFNULL(region.csd_uuid, country.csd_uuid))) AS partof FROM hippo_facility AS facility LEFT JOIN hippo_county AS county ON county.id = facility.location LEFT JOIN hippo_district AS district ON district.id = facility.location LEFT JOIN hippo_region AS region ON region.id = facility.location LEFT JOIN hippo_country AS country ON country.id = facility.location LEFT JOIN hippo_facility_type ft ON ft.id = facility.facility_type",
			'county' => "SELECT county.csd_uuid as uuid, DATE_FORMAT(county.last_modified, '%Y-%m-%d %T') AS lastupdated, county.id, county.name, 'nan' AS dhis_orgunit, 'na' AS facility_type, 'na' AS facility_type_name, district.csd_uuid AS partof FROM hippo_county AS county LEFT JOIN hippo_district AS district ON district.id = county.district",
			'district' => "SELECT district.csd_uuid as uuid, DATE_FORMAT(district.last_modified, '%Y-%m-%d %T') AS lastupdated, district.id, district.name, 'nan' AS dhis_orgunit, 'na' AS facility_type, 'na' AS facility_type_name, region.csd_uuid AS partof FROM hippo_district AS district LEFT JOIN hippo_region AS region ON region.id = district.region",
			'region' => "SELECT region.csd_uuid as uuid, DATE_FORMAT(region.last_modified, '%Y-%m-%d %T') AS lastupdated, region.id, region.name, 'nan' AS dhis_orgunit, 'na' AS facility_type, 'na' AS facility_type_name, country.csd_uuid AS partof FROM hippo_region AS region LEFT JOIN hippo_country AS country ON country.id = region.country",
			'country' => "SELECT country.csd_uuid as uuid, DATE_FORMAT(country.last_modified, '%Y-%m-%d %T') AS lastupdated, country.id, country.name,'nan' AS dhis_orgunit, 'na' AS facility_type, 'na' AS facility_type_name, null AS partof FROM hippo_country AS country",
		);
		...
		try {
			...
			while ($row = $stmt->fetch()) {
				...
				if ($row->dhis_orgunit == 'nan') {

				} else {
					$data['id_system_dhis2'] = 'https://hmis2.health.go.ug/';
					$data['id_code_dhis2'] = $row->dhis_orgunit;
				}
	...
	protected function create_Location($data, &$top) {
		...
		if ($this->useJSON) {
			...
			if (array_key_exists('id_system_dhis2', $data) && array_key_exists('id_code_dhis2', $data)) {
				$top['identifier'][] = array('system' => $data['id_system_dhis2'], 'value' => $data['id_code_dhis2']);
			}
			...
			if (array_key_exists('id_system_dhis2', $data) && array_key_exists('id_code_dhis2', $data)) {
				$sys = $this->doc->createElement("system");
				$sys->setAttribute('value', $data['id_system_dhis2']);
				$ident->appendChild($sys);
				$code = $this->doc->createElement("code");
				$code->setAttribute('value', $data['id_code_dhis2
                    ']);
				$ident->appendChild($code);
			}
			$top->appendChild($ident);
